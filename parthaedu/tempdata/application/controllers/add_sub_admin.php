...
class add_sub_admin extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data['sub_admin_detail']=	$this->sub_admin_model->sub_admin_edit_details('5');

        $this->load->view('admin/template/admin_header');
        $this->load->view('admin/template/admin_leftmenu');
        $this->load->view('admin/add_sub_admin_view',$data);
    ...
    public function sub_admin_record()
    {
        ...
        $sub_admin_email = $this->input->post('sub_admin_name_email');
        $sub_admin_password = $this->input->post('sub_admin_name_password');
        $sub_admin_status = $this->input->post('sub_admin_name_status');
        $logged_ip = $this->input->ip_address();

        $data['sub_admin_edit_details_by_user_name'] = $this->sub_admin_model->sub_admin_edit_details_by_user_name($this->input->post('sub_admin_name_user_name'));
        $data['sub_admin_edit_details_by_email'] = $this->sub_admin_model->sub_admin_edit_details_by_email($this->input->post('sub_admin_name_email'));
        ...
        if (count($sub_admin_edit_details_by_user_name) <= 0 && count($sub_admin_edit_details_by_email) <= 0) {
            ...
            (
                'first_name' => $sub_admin_name,
                'username' => $sub_admin_user_name,
                'email' => $sub_admin_email,
                'password' => $sub_admin_password,
                'status' => $sub_admin_status,
                'role_id' => 5,
                'lastlogon_datetime' => date("Y-m-d H:i:s"),
                'created_id' => $logged_ip
            );
            //echo "<pre>";print_r($sub_admin_data);exit;
            if ($this->db->insert('user', $sub_admin_data)) {

                $this->email->from('partha.logistics108@gmail.com');
                $this->email->to($sub_admin_email);
                $this->email->subject('Sub Admin Login Info.');
                $this->email->message('Dear ' . $sub_admin_name . ' Your Username: ' . $sub_admin_user_name . ' And Password: ' . $sub_admin_password . ' For The Role Of Sub Admin. Thanks! ');
            ...
            if (count($sub_admin_edit_details_by_user_name) > 0 && count($sub_admin_edit_details_by_email) > 0) {
                $this->session->set_flashdata('message', 'Sub-Admin [USER-NAME:' . $sub_admin_name . '] & [EMAIL:' . $sub_admin_email . '] Is Already Exist.');
            } else if (count($sub_admin_edit_details_by_user_name) > 0) {
                $this->session->set_flashdata('message', 'Sub-Admin [USER-NAME:' . $sub_admin_user_name . '] Is Already Exist.');
