...
class studentlist extends CI_Controller {
	...
	function edit_course_to_student()
	{
		...
		@$this->common_model->delete_data('tbl_add_course_to_student','add_course_id',$c_id);
		@$this->common_model->delete_data('tbl_add_course_subject_to_student','add_course_id',$c_id);
		@$this->common_model->delete_data('tbl_add_course_to_student_payment_details','course_id',$c_id);
...
//exit;
	...
	function payment_receipt($id)
	{
		...
		if($pay_status == 'paid' || $pay_status == '0' && $rec_no != '')
		{
...
//payment Received
	...
	public function excel_download()
	{
		...
		$this->excel->getActiveSheet()->setCellValue('K1', 'COURSE NAME');
		$this->excel->getActiveSheet()->setCellValue('L1', 'SUBJECT NAME');
		...
		for($counter=0;$counter<count($sub_admin_id_array);$counter++)
		{
			...
			if(!empty($course_details_by_id['c_id']))
			{
				foreach ($course_details_by_id['c_id'] as $c)
				{
					...
					$this->excel->getActiveSheet()->setCellValue("K" . $excel_id, $course_name);
				...
				foreach ($sub_details_by_id['s_id'] as $s_id)
				{
					...
					$this->excel->getActiveSheet()->setCellValue("L" . $excel_id, $subject_name);
					$excel_id++;
				}
...
//exit;
	...
	function refund_fee($id)
	{
		...
		// exit;
		redirect('studentlist/add_student_to_payment_view/'.$id,'refresh');
	}
