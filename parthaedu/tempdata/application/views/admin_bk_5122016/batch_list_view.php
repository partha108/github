  ...
  <div class="row-fluid sortable">
    <div class="box span12">
      <div class="box-header well" data-original-title>
        <h2><i class="icon-user"></i>Batch</h2>
      </div>
      <div class="box-content">
       ...
       <div>
           ...
           <a class="btn btn-primary" href="javascript:void(0)" onclick="return open_add_model()" > Add Batch </a>
       ...
       <i class="icon-edit icon-white"></i> In-Active </a></span>
        ...
        <table class="table table-striped table-bordered">
          ...
          <tr>
           ...
           {
                    $course_id = @$sub_name->course_name;
                    @$course_details = $this->common_model->add_course_data('tbl_course','course_id',$course_id);
               // print_r($sub_name);
              $count = $count+1;
          ?>
           <td style="width:1px"><input type="checkbox" name="check_id" id="<?php echo $sub_name->batch_id;?>"  class="chtest_test" 
           onClick="single_check_box_checked(this.checked,this.id)" value="<?php echo $sub_name->batch_id;?>"/></td>
          <td><?php echo $count; ?></td>
              <td><?php echo $sub_name->session;?></td>
              <td><?php echo $course_details[0]->course_name	;?></td>
              <td><?php echo $sub_name->batch_class_name; ?></td>
          <td><?php echo $sub_name->batch_name;?></td>
          ...
          <td class="center"><a class="btn btn-info" href="#" onclick="return go_to_view('<?php echo $sub_name->batch_id;?>')"> <i class="icon-edit icon-white"></i> Edit </a>

              <a class="btn btn-danger" href="#" onclick="delete_batch(<?php echo $sub_name->batch_id ;?>)"> <i class="icon-trash icon-white"></i> Delete </a>
               </td>
          </tr>
          <?php
           }
...
<div class="modal hide fade" id="myaddmodel" style="width:1000px; left:40%">
    <?php echo form_open_multipart('batch_module/add_batch', array('class' => 'form-horizontal', 'id' => 'addSubjectFrm')); ?>
        ...
        <div style="width:500px;  float:left;">
            <?php echo $this->session->flashdata('insert_message');?>
                <div class="control-group" id="name_control" >
                    ...
                    <div class="controls">
                        <select  id="add_ac_year" name="add_ac_year" onchange="show_course(this.value)">
                ...
                <div class="control-group" id="name_control" >
                    ...
                    <div class="controls">
                        <select id="db_course" name="add_course" onchange="show_class(this.value);">
                ...
                <div class="control-group" id="name_control" >
                    ...
                    <div class="controls">
                        <!--<select id="selectclassbyacademic"  multiple="multiple" name="course_class1[]">
                ...
                <div class="control-group" id="name_control" >
                    <label class="control-label" for="inputSuccess"> Batch Name</label>
                    <div class="controls">
                        <input type="text" id="add_batch_name"  name="add_batch_name" >
                        <span class="help-inline" id="name_message" style="display:none;"></span>
                    </div>
                </div>
...
{
    //alert(id)
    $("#class_id").load('<?php echo base_url();?>index.php/batch_module/add_class/'+id);
    //$(".ms-drop").load('<?php echo base_url();?>index.php/batch_module/course_class_ul/'+id);
...
{
    //alert(id)
    window.location = base_url+'index.php/batch_module/batch_edit_view/'+id;
}
...
{//alert(id)
  ...
  if(id){
    ...
    $.ajax({
          type: "POST",
          dataType:'json',  
          url:base_url+"index.php/subject_module/edit_fees", 
          data: dataString,
          async: false,  
         success: function(data) {             
           var edit_fees=data.edit_fees[0];   
            $("#id").val(id);
           // $("#edit_ac_year").val(edit_fees.session);
           // $("#edit_db_course").val(edit_fees.course_name);
            $("#edit_batch_name").val(edit_fees.batch_name);
...
<script language="javascript" type="text/javascript">
    ...
    function filterInactiveActive(id)
    {
        ...
        window.location = base_url+'index.php/batch_module/'+id;
    ...
    function delete_data(id)
    {
        if(confirm('Are you sure do you want to delete it?'))
        {
            window.location = base_url+'index.php/batch_module/sub_admin_delete/'+id;
        }
    }
    ...
    {
        $.post(base_url+'index.php/batch_module/sub_admin_active_inactive/',
            {
                id:id,
                value:value
            },
...
<script>
    ...
    function active_sub_admin()
    {
        ...
        if(checkboxValue!='')
        {
            $.post(base_url+'index.php/batch_module/sub_admin_active_more_than_one_id/',
                {
    ...
    {	//alert("hi");
        ...
        {
            //alert(checkboxValue);
            $.post(base_url+'index.php/batch_module/sub_admin_in_active_more_than_one_id/',
                {
                    sub_admin_id:checkboxValue
                },
                function(data,status)
    ...
    function delete_batch(id)
    {
        ...
        if(id)
        {
            ...
            if(confirm('Are you sure do you want to delete this Academic Year?')){
                $.ajax({
                    type:"POST",
                    url: "<?php echo base_url() ?>index.php/batch_module/delete_batch",
                    data:{deleteid:id},
    ...
    function pagination_select(value,id)
    {
        ...
        if(value.trim())
        {
            if(value.trim())
            {
                ...
                window.location = base_url+'index.php/batch_module?'+str+'&'+id+'='+value;
            }
            else
            {
                window.location = base_url+'index.php/batch_module?'+str;
            }

        }
        else
        {
            window.location = base_url+'index.php/batch_module?'+id+'='+value;
        }
