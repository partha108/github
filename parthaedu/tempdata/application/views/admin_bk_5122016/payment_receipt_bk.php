...
<?php
function convert_amt_to_words($number)
{
	...
	if (($number >= 0 && (int) $number < 0) || (int) $number < 0 - PHP_INT_MAX) {
		// overflow
		trigger_error(
			'convert_amt_to_words only accepts numbers between -' . PHP_INT_MAX . ' and ' . PHP_INT_MAX,
			E_USER_WARNING
		);
		return false;
	}
	...
	if ($number < 0) {
		return $negative . convert_amt_to_words(abs($number));
	}
	...
	switch (true) {
		...
		case $number < 1000:
			...
			if ($remainder) {
				$string .= $conjunction . convert_amt_to_words($remainder);
			}
			break;
		default:
			...
			$string = convert_amt_to_words($numBaseUnits) . ' ' . $dictionary[$baseUnit];
			if ($remainder) {
				$string .= $remainder < 100 ? $conjunction : $separator;
				$string .= convert_amt_to_words($remainder);
			}
			break;
	}
...
//echo convert_amt_to_words(1800);
?>
<?php
$total_fees =0;
$total_vat_amt =0;
...
<center>
	<div style="border: 1px solid black;width: 60%">
		...
		<table width="100%" class="tbl_font_size">
			...
			<tr>
				<td colspan="4" class="receipt">Total Amount Received:<?php echo $total_amt." (".convert_amt_to_words($total_amt).")"; ?></td>
			</tr>
...
<center>
	...
	</div><a href="javascript:void(0)" onclick="print_data()">Print</a>
</center>
