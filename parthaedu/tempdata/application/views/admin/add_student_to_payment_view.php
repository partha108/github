                    ...
                    foreach($details as $cd) //1st
                    {
                       ...
                       // print_r($details);
                        foreach ($stu_details as $sd) {
                            ...
                            @$class_name_id = $acd_year[0]->class_name;

                            @$course_name = $this->common_model->add_course_data('tbl_course','course_id',$course_name_id);
                            @$details_sub_name = $this->common_model->payment_head_batch_detail('tbl_add_course_subject_to_student','add_course_id',$course_id,'student_id',$student_id,'subject_name',$subject_id);
                            ...
                            <tr>
                                ...
                                <td><?php if(@$cd->payment_head_name == "Exam Fees" || $cd->payment_head_name == "Reg Fees" || $cd->payment_head_name == "Discount" || $cd->payment_head_name == "Add_fee"){echo '-';}else{echo $details_sub_name[0]->batch_name;}?></td>
                                ...
                                <td style="text-align:center" >
                                    <?php
                                        if(@$cd->payment_status=='paid' && @$cd->payment_mode=='cash' && @$cd->check_status  == '0')
                                        {
                                    ?>
                                            <span class="label label-success">Cash</span>
                                    <?php
                                        }
                                        else if(@$cd->payment_status=='paid'  && @$cd->payment_mode=='chaque' && @$cd->check_status  == '1' || (@$cd->payment_mode=='cash' && @$cd->check_status  == '0'))
                                        {
                                    ?>
                                            <span class="label label-success">Cleared</span>
                                    <?php
                                        }
                                        else if(@$cd->payment_status =='0' && @$cd->payment_mode=='chaque' && @$cd->check_status  == '1')
                                        {
                                    ?>
                                        <span class="label label-success">Canceled</span>
                                    <?php
                                        }
                                        else if(@$cd->payment_status=='pending' && @$cd->payment_mode=='chaque' && @$cd->check_status  == '2')
                                        {
                                    ?>
                                        <span class="label label-important">Bounced</span>
                                    <?php
                                        }
                                        else if(@$cd->payment_status=='pending' && @$cd->payment_mode=='chaque' && @$cd->check_status  == '3')
                                        {
                                    ?>
                                ...
                                <td>
                                    <?php
                                        if($cd->payment_status == 'pending' && $cd->payment_mode == 'chaque' && $cd->check_status  == '3')
                                        {
                                    ?>
                                            <a class="btn btn-info" onclick="return open_recipet(<?php echo $cd->payment_id;?>)" href="#" title="Ack print">
                                                <i class="icon-print icon-white"></i>
                                            </a>
                                    <?php
                                        }
                                        else if(@trim($cd->payment_status)=='paid' && (@trim($cd->payment_mode) == 'cash' || trim($cd->payment_mode)=='chaque') && (@trim($cd->check_status)  == '0' || trim($cd->check_status)  == '1'))
                                        {
                                    ?>
                                    ...
                                    <?php
                                        ...
                                        else if(@trim($cd->payment_status)=='0' && (@trim($cd->payment_mode) == 'cash' || trim($cd->payment_mode)=='chaque') && (@trim($cd->check_status)  == '0' || trim($cd->check_status)  == '1'))
                                        {
                                    ?>
                                        <a class="btn btn-info" onclick="return open_recipet(<?php echo $cd->payment_id;?>)" href="#" title="cancel Receipt">
                                                <i class="icon-print icon-white"></i>
                                            </a>
                                    <?php
                                    }
                                ...
                                <td> <a class="btn btn-danger" href="#" title="Delete" onclick="delete_payment_id(<?php echo $cd->payment_id ;?>)"> <i class="icon-trash icon-white"></i> </a></td>
                            </tr>
...
<script language="javascript" type="text/javascript">
    ...
    function delete_payment_id(id)
    {
        if(confirm('Are you sure do you want to delete it?'))
        {
            window.location = base_url+'index.php/studentlist/delete_payment_id/'+id;
